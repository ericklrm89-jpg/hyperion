/**
 * HYPERION v11 — Instagram Reels Publisher (SPANISH - PRODUCTION HYDRA PROOF)
 */
...
async function main() {
  console.log('\n============================================');
  console.log('🚀 HYPERION v11 — Instagram Reels Spanish (Production)');
  console.log('============================================');
  ...
  console.log('\n🆕 STEP 0: Navigating to Instagram...');
  await cdpCall(ws, 'Runtime.evaluate', { expression: "window.onbeforeunload = null; window.location.href='https://www.instagram.com/';" });
  await wait(7000);

  // STEP 1: Click Create
  console.log('\n➕ STEP 1: Opening Instagram Create Modal...');
  const createBtn = await cdpCall(ws, 'Runtime.evaluate', {
    expression: `(() => {
  ...
  if (createBtn.result?.value) {
    const p = JSON.parse(createBtn.result.value);
    await mouseClick(ws, p.x, p.y);
  } else {
    await mouseClick(ws, 24, 460);
  }
  await wait(4000);
  ...
  console.log('   ✅ Video injected. Waiting 8s for preview...');
  await wait(8000);
  await screenshot(ws, 'C:\\Users\\erick\\.gemini\\antigravity-ide\\scratch\\hyperion-web-agent\\ig_prod_01_uploaded.png');

  // STEP 3: Click Next -> Next
  console.log('\n▶️ STEP 3: Clicking Next / Siguiente (1 & 2)...');
  for (let s = 1; s <= 2; s++) {
  ...
  await screenshot(ws, 'C:\\Users\\erick\\.gemini\\antigravity-ide\\scratch\\hyperion-web-agent\\ig_prod_02_caption.png');
  ...
  console.log('\n🚀 STEP 5: Clicking Compartir / Share...');
  ...
  console.log('   ⏳ Waiting 15s for Instagram success render...');
  await wait(15000);
  await screenshot(ws, 'C:\\Users\\erick\\.gemini\\antigravity-ide\\scratch\\hyperion-web-agent\\ig_prod_03_published.png');

  ws.close();
  console.log('\n✅ Instagram Reels (Spanish) PUBLISHED AND VERIFIED!');
}

main().catch(e => { console.error('❌ FATAL Instagram ES:', e.message); process.exit(1); });
