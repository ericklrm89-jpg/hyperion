/**
 * HYPERION v11 — Instagram Reels Publisher (PORTUGUESE - CLEAN FRESH MODAL)
 */
const http = require('http');
const WebSocket = require('ws');
const fs = require('fs');

const COPY_IG = `🚀 Quer fazer suas redes sociais crescerem e atrair milhares de seguidores reais?
...
async function main() {
  console.log('\n════════════════════════════════════════════');
  console.log('🚀 HYPERION v11 — Instagram Reels Portuguese (CLEAN FLOW)');
  console.log('════════════════════════════════════════════');
    ...
    expression: `(() => {
      const btn = Array.from(document.querySelectorAll('a, button, div[role="button"]')).find(e => {
        ...
        return r.width > 0 && r.left < 100 && (
          txt.includes('new post') || txt.includes('create') || txt.includes('criar') ||
  ...
  console.log('\n📤 STEP 3: Injecting Portuguese video into input[type=file]...');
  ...
  await wait(8000);
  await screenshot(ws, 'C:\\Users\\erick\\.gemini\\antigravity-ide\\scratch\\hyperion-web-agent\\ig_03_video_pt.png');

  // STEP 4-5: Click Siguiente -> Siguiente
  console.log('\n▶️ STEP 4-5: Clicking Siguiente / Next / Avançar...');
    ...
    const nextBtn = await cdpCall(ws, 'Runtime.evaluate', {
      expression: `(() => {
        const btns = Array.from(document.querySelectorAll('div[role="button"], button, span, a')).filter(e => {
          const txt = (e.textContent || '').trim().toLowerCase();
          const r = e.getBoundingClientRect();
          return (txt === 'next' || txt === 'siguiente' || txt === 'avançar') && r.width > 0;
        });
        if (btns.length > 0) {
  ...
  // STEP 6: Write Portuguese caption
  console.log('\n✍️ STEP 6: Writing Portuguese caption...');
  await cdpCall(ws, 'Runtime.evaluate', {
    expression: `(() => {
  ...
  const shareBtn = await cdpCall(ws, 'Runtime.evaluate', {
    expression: `(() => {
      const btn = Array.from(document.querySelectorAll('div[role="button"], button, span')).find(e => {
        const txt = (e.textContent || '').trim().toLowerCase();
        const r = e.getBoundingClientRect();
        return (txt === 'share' || txt === 'compartir' || txt === 'compartilhar') && r.width > 0;
      });
      if (btn) {
        const r = btn.getBoundingClientRect();
        return JSON.stringify({ x: Math.round(r.left + r.width/2), y: Math.round(r.top + r.height/2) });
  ...
  await screenshot(ws, 'C:\\Users\\erick\\.gemini\\antigravity-ide\\scratch\\hyperion-web-agent\\ig_06_publicado_pt.png');

  ws.close();
  console.log('\n✅ Instagram Reel (Portuguese) PUBLISHED!');
}
