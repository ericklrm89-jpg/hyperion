...
async function main() {
  ...
  console.log('Clicking x=490, y=624 (Seleccionar del ordenador)...');
  await cdpCall('Input.dispatchMouseEvent', { type: 'mouseMoved', x: 490, y: 624 });
  await new Promise(r => setTimeout(r, 80));
  await cdpCall('Input.dispatchMouseEvent', { type: 'mousePressed', x: 490, y: 624, button: 'left', clickCount: 1 });
  await new Promise(r => setTimeout(r, 80));
  await cdpCall('Input.dispatchMouseEvent', { type: 'mouseReleased', x: 490, y: 624, button: 'left', clickCount: 1 });

  const backendNodeId = await fileChooserPromise;
  console.log('Injecting file to backendNodeId:', backendNodeId);
  ...
  fs.writeFileSync('C:\\Users\\erick\\.gemini\\antigravity-ide\\scratch\\hyperion-web-agent\\ig_click_test.png', Buffer.from(ss.data, 'base64'));
  console.log('📸 Screenshot saved: ig_click_test.png');
