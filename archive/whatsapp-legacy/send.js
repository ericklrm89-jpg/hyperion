/**
 * Módulo para WhatsApp Web — Hyperion Engine v3
 */
const fs = require('fs');
  ...
  return new Promise((resolve, reject) => {
    http.get('http://localhost:9222/json', (res) => {
      ...
      res.on('end', () => {
        ...
        if (!waTab) reject(new Error('Pestaña de WhatsApp Web no encontrada en Chrome'));
        else resolve(waTab);
      });
    }).on('error', reject);
  });
...
async function sendWhatsAppMessage(contactName, messageText) {
  console.log('═══════════════════════════════════════════════════');
  console.log('  HYPERION MODULE — WhatsApp Web Send');
  console.log('═══════════════════════════════════════════════════');

  const tab = await getTab();
  const ws = new WebSocket(tab.webSocketDebuggerUrl);
  await new Promise((r, e) => { ws.on('open', r); ws.on('error', e); });
  console.log(`✅ Conectado a WhatsApp Web: ${tab.title}`);

  // Inyectar overlay Hyperion v3 para mapear elementos de WhatsApp Web
  await evalJS(ws, ORIGINAL_HYPERION_OVERLAY_SCRIPT, 20000);
  await sleep(800);
  const data = JSON.parse(await evalJS(ws, 'window.__hyData()') || '{}');
  console.log(`📋 Capa WhatsApp: ${data.type} | Elementos: ${data.elements?.length || 0}`);

  ws.close();
  console.log('🎉 Flujo WhatsApp completado');
