/**
 * HYPERION v11 — Gemini Web Flow (SPANISH - PRODUCTION)
 * Follows 100% SKILL.md gemini.md guidelines for Spanish Video Generation
 */
...
FILES.forEach(f => {
  if (!fs.existsSync(f)) {
    console.error(`❌ Spanish input storyboard missing: ${f}`);
    process.exit(1);
  }
});

const PROMPT_ES = "Anima estas 3 imágenes de storyboard de marketing subidas para la aplicación de sorteos FairDraw. Genera un video promocional vertical 9:16 de 10 segundos de alta energía adecuado para Instagram Reels y TikTok. La animación debe realizar una transición suave entre estas escenas, mostrando un sistema provadamente justo y confiable. Incluye una locución enérgica en español (neutro o latino) y subtítulos dinámicos sincronizados en pantalla. Mantén el logotipo oficial de FairDraw consistente en todo momento.";

async function main() {
  console.log('\n═══════════════════════════════════════════════');
  console.log('🚀 HYPERION v11 — Gemini Spanish (PRODUCTION)');
  console.log('═══════════════════════════════════════════════');
  ...
  await wait(600);
  await screenshot(ws, 'C:\\Users\\erick\\.gemini\\antigravity-ide\\scratch\\hyperion-web-agent\\hy_01_manus_es.png');
  ...
  // STEP 3: Click (+) with dynamic DOM coords
  console.log('\n🔧 STEP 3: Clicking (+) button...');
  const plusCoords = await cdpCall(ws, 'Runtime.evaluate', {
    expression: `(() => {
      const b = Array.from(document.querySelectorAll('button')).find(b => {
        const a = (b.getAttribute('aria-label')||'').toLowerCase();
        return a.includes('carga') || a.includes('subida') || a.includes('upload');
      });
      if (!b) return null;
  ...
  // STEP 4: Search for "Subir archivos"
  console.log('\n📂 STEP 4: Locating "Subir archivos" menu option...');
  let pUp = null;
  ...
  if (!pUp) throw new Error('❌ "Subir archivos" option not found in popup');
  console.log(`   "Subir archivos" Coords: x=${pUp.x}, y=${pUp.y}`);

  // STEP 5: Register listener RIGHT BEFORE click to avoid timeout!
  console.log('\n🎣 STEP 5: Registering fileChooser listener & clicking "Subir archivos"...');
  const fileChooserPromise = new Promise((resolve, reject) => {
    ...
    ws.on('message', h);
    setTimeout(() => { ws.removeListener('message', h); reject(new Error('Timeout: fileChooserOpened (15s)')); }, 15000);
  });

  await mouseClick(ws, pUp.x, pUp.y);

  // STEP 6: Inject files via backendNodeId
  console.log('\n📤 STEP 6: Waiting for interception and injecting 3 storyboards...');
  ...
  console.log('   ✅ 3 images injected. Waiting for thumbnail render (6s)...');
  await wait(6000);
  await screenshot(ws, 'C:\\Users\\erick\\.gemini\\antigravity-ide\\scratch\\hyperion-web-agent\\hy_03_archivos_es.png');

  // STEP 7: Type Spanish prompt
  console.log('\n✍️ STEP 7: Injecting Spanish prompt...');
  await cdpCall(ws, 'Runtime.evaluate', {
    expression: `(() => {
      ...
      if (ed) {
        ...
        document.execCommand('insertText', false, ${JSON.stringify(PROMPT_ES)});
  ...
  await wait(1000);
  await screenshot(ws, 'C:\\Users\\erick\\.gemini\\antigravity-ide\\scratch\\hyperion-web-agent\\hy_04_prompt_es.png');
  ...
  await wait(5000);
  await screenshot(ws, 'C:\\Users\\erick\\.gemini\\antigravity-ide\\scratch\\hyperion-web-agent\\hy_05_generando_es.png');
  await wait(85000);

  // STEP 9: Download video
  console.log('\n⬇️ STEP 9: Searching for video download button...');
  await screenshot(ws, 'C:\\Users\\erick\\.gemini\\antigravity-ide\\scratch\\hyperion-web-agent\\hy_06_video_listo_es.png');
  ...
  if (dlPos.result?.value) {
    ...
    await wait(3000);
    console.log('   ✅ Spanish Video downloaded into C:\\Users\\erick\\Downloads!');
  } else {
    console.log('   ⚠️ Download button not found yet. Check hy_06_video_listo_es.png');
  }

  ws.close();
  console.log('\n✅ ═══════════════════════════════════════════════');
  console.log('   HYPERION v11 — Spanish Gemini Flow COMPLETED');
  console.log('═══════════════════════════════════════════════\n');
}
