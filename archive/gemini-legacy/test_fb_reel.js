...
http.get('http://localhost:9222/json', r => {
  ...
  r.on('end', async () => {
    ...
    // Navegar a Reels Create
    console.log('Navigating to reels/create...');
    await send('Page.navigate', { url: 'https://www.facebook.com/reels/create' });

    // Esperar a que la página cargue e hidratación
    console.log('Waiting for load...');
    await new Promise(r => setTimeout(r, 6000));

    // Inyectar Manus
    await send('Runtime.evaluate', { expression: MANUS });
    await new Promise(r => setTimeout(r, 500));

    // Tomar screenshot
    const ss = await send('Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync('fb_reels_create.png', Buffer.from(ss.data, 'base64'));
    console.log('Screenshot saved to fb_reels_create.png');

    // Listar todos los elementos interactivos visibles
