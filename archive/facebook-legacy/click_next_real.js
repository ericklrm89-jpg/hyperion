...
async function main() {
  ...
  console.log('Clicking Next blue button at x=197, y=780...');
  await mouseClick(197, 780);
  await new Promise(r => setTimeout(r, 3000));

  const ss1 = await cdpCall('Page.captureScreenshot', { format: 'png' });
  fs.writeFileSync('C:\\Users\\erick\\.gemini\\antigravity-ide\\scratch\\hyperion-web-agent\\fb_step3_caption_screen.png', Buffer.from(ss1.data, 'base64'));
  ...
  console.log('Clicking Post blue button at x=197, y=780...');
  await mouseClick(197, 780);
  await new Promise(r => setTimeout(r, 12000));

  const ss2 = await cdpCall('Page.captureScreenshot', { format: 'png' });
  fs.writeFileSync('C:\\Users\\erick\\.gemini\\antigravity-ide\\scratch\\hyperion-web-agent\\fb_published_real_done.png', Buffer.from(ss2.data, 'base64'));
  console.log('📸 Screenshot saved: fb_published_real_done.png');
