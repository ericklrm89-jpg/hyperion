...
http.get('http://localhost:9222/json', r => {
  ...
  r.on('end', async () => {
    ...
    console.log('📤 Inyectando video vertical...');
    await cdpCall(ws, 'DOM.setFileInputFiles', { backendNodeId, files: [VIDEO_PATH] });
    console.log('✅ Video inyectado.');

    console.log('⏳ Esperando procesamiento de video en historia (10s)...');
    await wait(10000);

    // Capturar pantalla de previsualización
    const ss = await cdpCall(ws, 'Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync('fb_story_video_preview.png', Buffer.from(ss.data, 'base64'));
    console.log('📸 fb_story_video_preview.png guardada.');
    ...
    console.log('\n=== ELEMENTOS DETECTADOS POST-VIDEO ===');
