...
http.get('http://127.0.0.1:9222/json', r => {
  ...
  r.on('end', async () => {
    ...
    ws.on('open', async () => {
      ...
      // Escribir "Fabro" en el input real
      const writeRes = await cdpCall(ws, 'Runtime.evaluate', {
        expression: `(() => {
          const inp = document.querySelector('input[placeholder*="Buscar"]') || document.querySelector('input[placeholder*="Search"]') || document.querySelector('#_r_a_');
          if (inp) {
            inp.focus();
            inp.value = '';
            inp.value = 'Fabro';
      ...
      console.log('Status de escritura:', writeRes.result?.value);
      await wait(3000);
