            ...
            try {
                switch (name) {
                    case 'browser_navigate': {
                        ...
                        await this.hyperion.navigate.navigate({ url, waitUntil: waitUntil });
                    ...
                    case 'browser_click': {
                        ...
                        await this.hyperion.click.click(selector, { button: button, clickCount });
                        return { content: [{ type: 'text', text: `Clicked ${selector}` }] };
                    }
                    case 'browser_type': {
                        const selector = args.selector;
                        const text = args.text;
                        const clearField = args.clearField || false;
                        const humanLike = args.humanLike !== false;
                        const pasteThreshold = args.pasteThreshold || 100;
                    ...
                    case 'browser_handle_dialog': {
                        ...
                        await this.hyperion.dialog.handleDialog(action, promptText);
                    ...
                    case 'browser_overlay_inject': {
                        await this.hyperion.overlay.inject({
                            intervalMs: args.intervalMs || 2000,
                            includePostLinks: args.includePostLinks !== false,
                            gridSize: args.gridSize || 200
                        });
