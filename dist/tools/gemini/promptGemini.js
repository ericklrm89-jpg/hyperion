...
exports.promptGeminiAction = exports.promptGeminiSchema = void 0;
exports.executePromptGemini = executePromptGemini;
 ...
 */
exports.promptGeminiSchema = zod_1.z.object({
    prompt: zod_1.z.string().min(1, 'Prompt text is required').describe('Prompt instruction to send to Gemini Web'),
    imagePaths: zod_1.z.array(zod_1.z.string()).optional().describe('Optional list of absolute file paths to attach as images/assets'),
});
/**
 * Action Definition for ActionRegistry
 */
exports.promptGeminiAction = {
    id: 'gemini-prompt',
    ...
    schema: exports.promptGeminiSchema,
    perception: 'visual',
 ...
 */
async function executePromptGemini(cxn, rawInput, options) {
    const input = exports.promptGeminiSchema.parse(rawInput);
    const sleep = options?.sleepFn || ((ms) => new Promise(r => setTimeout(r, ms)));
    logger_1.logger.info({ prompt: input.prompt.slice(0, 50), imagesCount: input.imagePaths?.length || 0 }, '[Gemini] Starting prompt flow');
    ...
    if (input.imagePaths && input.imagePaths.length > 0) {
        ...
        if (queryRes?.nodeId) {
            ...
            logger_1.logger.info({ count: input.imagePaths.length }, '[Gemini] Reference images injected into file input');
            await sleep(3000);
        }
    }
  ...
  `);
    logger_1.logger.info('[Gemini] Prompt sent. Waiting for generation response...');
...
//# sourceMappingURL=promptGemini.js.map