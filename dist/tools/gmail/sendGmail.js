...
exports.sendGmailAction = exports.sendGmailSchema = void 0;
exports.executeSendGmail = executeSendGmail;
 ...
 */
exports.sendGmailSchema = zod_1.z.object({
    recipient: zod_1.z.string().email('Valid recipient email required').describe('Target corporate or client email address'),
    subject: zod_1.z.string().min(1, 'Subject is required').describe('Email subject line'),
    bodyHtml: zod_1.z.string().min(1, 'HTML body is required').describe('Rich HTML formatted email body with embedded CSS and tables'),
    attachmentPath: zod_1.z.string().optional().describe('Optional absolute path to an image/PDF file to attach'),
});
/**
 * Action Definition for ActionRegistry
 */
exports.sendGmailAction = {
    id: 'gmail-send',
    ...
    schema: exports.sendGmailSchema,
    perception: 'visual',
 ...
 */
async function executeSendGmail(cxn, rawInput, options) {
    const input = exports.sendGmailSchema.parse(rawInput);
    const sleep = options?.sleepFn || ((ms) => new Promise(r => setTimeout(r, ms)));
    logger_1.logger.info({ recipient: input.recipient, subject: input.subject }, '[Gmail] Starting outbound send flow');
    ...
    if (input.attachmentPath) {
        logger_1.logger.info({ file: input.attachmentPath }, '[Gmail] Inyectando adjunto mediante DOM.setFileInputFiles');
