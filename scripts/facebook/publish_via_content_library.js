/**
 * HYPERION — Facebook Official Content Library Publisher
 */
...
async function main() {
  console.log('\n============================================');
  console.log('🚀 HYPERION — Facebook Official Content Library Upload');
  console.log('============================================');
  ...
  // STEP 1: Click + Create Reel in Content Library
  console.log('\n➕ STEP 1: Clicking + Create button in Content Library...');
  const createBtn = await cdpCall(ws, 'Runtime.evaluate', {
    expression: `(() => {
      const btns = Array.from(document.querySelectorAll('button, div[role="button"]')).filter(e => {
        ...
        return txt.includes('create') || txt.includes('crear');
      });
      if (btns.length > 0) {
        const b = btns[0];
  ...
  if (createBtn.result?.value) {
    ...
    console.log(`   Create btn clicked at x=${p.x}, y=${p.y}`);
    await mouseClick(ws, p.x, p.y);
    await wait(3000);
  }

  // Check dropdown options if any, or file input
  console.log('\n📤 STEP 2: Injecting video file into Content Library file input...');
  ...
  await wait(10000);
  await screenshot(ws, 'C:\\Users\\erick\\.gemini\\antigravity-ide\\scratch\\hyperion-web-agent\\fb_lib_01_uploaded.png');

  // Click Next (1 & 2)
  console.log('\n▶️ STEP 3: Clicking Siguiente / Next...');
  for (let s = 1; s <= 2; s++) {
  ...
  // Press Escape to dismiss hashtag dropdown
  ...
  console.log('   ⏳ Waiting 15s for Content Library refresh...');
  await wait(15000);

  await screenshot(ws, 'C:\\Users\\erick\\.gemini\\antigravity-ide\\scratch\\hyperion-web-agent\\fb_lib_02_POSTED_VERIFIED.png');
  ws.close();
  console.log('\n✅ Content Library upload finished!');
}
