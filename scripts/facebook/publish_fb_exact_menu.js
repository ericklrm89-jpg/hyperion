/**
 * HYPERION — Click Content Library + Create Menu on Facebook
 */
...
async function main() {
  ...
  console.log('1. Clicking + Create button at x=525, y=385...');
  await mouseClick(ws, 525, 385);
  await wait(3000);

  let ss = await cdpCall(ws, 'Page.captureScreenshot', { format: 'png' });
  fs.writeFileSync('C:\\Users\\erick\\.gemini\\antigravity-ide\\scratch\\hyperion-web-agent\\fb_EXACT_CREATE_DROPDOWN.png', Buffer.from(ss.data, 'base64'));
  console.log('Saved fb_EXACT_CREATE_DROPDOWN.png');

  console.log('2. Clicking Create reel item in dropdown...');
  await cdpCall(ws, 'Runtime.evaluate', {
    expression: `(() => {
      const items = Array.from(document.querySelectorAll('div[role="menuitem"], a, button, span')).filter(e => {
        const txt = (e.textContent || '').trim().toLowerCase();
        return txt.includes('reel') || txt.includes('video') || txt.includes('publicación') || txt.includes('post');
      });
  ...
  console.log('3. Injecting video file...');
  ...
  // Click Publish
  console.log('4. Clicking Publish button...');
  await cdpCall(ws, 'Runtime.evaluate', {
    expression: `(() => {
      const btns = Array.from(document.querySelectorAll('div[role="button"], button, span')).filter(e => {
        const txt = (e.textContent || '').trim().toLowerCase();
  ...
  console.log('   Waiting 15s for publish...');
  await wait(15000);

  ss = await cdpCall(ws, 'Page.captureScreenshot', { format: 'png' });
  fs.writeFileSync('C:\\Users\\erick\\.gemini\\antigravity-ide\\scratch\\hyperion-web-agent\\fb_EXACT_PUBLISHED_RESULT.png', Buffer.from(ss.data, 'base64'));
  console.log('Saved fb_EXACT_PUBLISHED_RESULT.png');
  ws.close();
}
