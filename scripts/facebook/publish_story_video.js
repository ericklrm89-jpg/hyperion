...
http.get('http://localhost:9222/json', r => {
  ...
  r.on('end', async () => {
    ...
    console.log('Clic en Share to story (Video):', pc);
    await mouseClick(ws, pc.x, pc.y);
    await wait(8000); // Dar un poco de tiempo extra al procesamiento de video

    // Tomar screenshot final
    await cdpCall(ws, 'Page.enable');
    const ss = await cdpCall(ws, 'Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync('fb_story_video_published.png', Buffer.from(ss.data, 'base64'));
    console.log('Saved fb_story_video_published.png');
