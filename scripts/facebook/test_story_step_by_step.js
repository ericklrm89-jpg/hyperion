...
async function main() {
  if (!fs.existsSync(IMAGE_PATH)) {
    console.error('❌ Archivo de imagen no encontrado:', IMAGE_PATH);
    process.exit(1);
  }

  console.log('🔍 Buscando pestaña de Facebook...');
  ...
  if (!tab) {
    console.error('❌ No se encontró pestaña de Facebook abierta.');
    process.exit(1);
  }
  ...
  console.log('📤 Inyectando imagen vertical 9:16...');
  await cdpCall(ws, 'DOM.setFileInputFiles', { backendNodeId, files: [IMAGE_PATH] });
  console.log('✅ Imagen inyectada.');

  console.log('⏳ Esperando procesamiento de la historia (6s)...');
  await wait(6000);

  // Capturar pantalla de previsualización
  const ss = await cdpCall(ws, 'Page.captureScreenshot', { format: 'png' });
  fs.writeFileSync('fb_story_preview_test.png', Buffer.from(ss.data, 'base64'));
  console.log('📸 fb_story_preview_test.png guardado.');

  // Obtener elementos de la pantalla posterior
  ...
  console.log('\n=== ELEMENTOS EN DETECTADOS ===');
  console.log(dump?.result?.value || '(ninguno)');
