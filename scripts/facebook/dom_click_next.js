...
async function main() {
  ...
  console.log('DOM clicking Next button...');
  ...
  fs.writeFileSync('C:\\Users\\erick\\.gemini\\antigravity-ide\\scratch\\hyperion-web-agent\\fb_dom_next_clicked.png', Buffer.from(ss1.data, 'base64'));
  ...
  console.log('DOM clicking Publish button...');
  await cdpCall('Runtime.evaluate', {
    expression: `(() => {
      const btns = Array.from(document.querySelectorAll('div[role="button"], button'));
      const target = btns.find(e => {
  ...
  fs.writeFileSync('C:\\Users\\erick\\.gemini\\antigravity-ide\\scratch\\hyperion-web-agent\\fb_dom_published_done.png', Buffer.from(ss2.data, 'base64'));
  console.log('📸 Screenshot saved: fb_dom_published_done.png');
