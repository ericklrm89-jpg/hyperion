/**
 * HYPERION v11 — Facebook Reels Publisher (English - Master)
 */
const http = require('http');
const WebSocket = require('ws');
const fs = require('fs');

const COPY_FB = `🚀 How to gain 10K real followers and scale your social reach fast!
...
async function main() {
  console.log('\n════════════════════════════════════════════');
  console.log('🚀 HYPERION v11 — Facebook Reels English');
  console.log('════════════════════════════════════════════');
  ...
  console.log('   ✅ English Video injected. Waiting for upload & draft render (10s)...');
  await wait(10000);
  await screenshot(ws, 'C:\\Users\\erick\\.gemini\\antigravity-ide\\scratch\\hyperion-web-agent\\fb_en_cargado.png');
  ...
  console.log('\n✍️ STEP 3: Writing English caption...');
  await cdpCall(ws, 'Runtime.evaluate', {
    expression: `(() => {
  ...
  // STEP 4: Click Publish / Post via DOM target
  console.log('\n🚀 STEP 4: Clicking Publish / Post button...');
  await cdpCall(ws, 'Runtime.evaluate', {
    expression: `(() => {
      const btns = Array.from(document.querySelectorAll('div[role="button"], button'));
      const target = btns.find(e => {
  ...
  await wait(12000);
  await screenshot(ws, 'C:\\Users\\erick\\.gemini\\antigravity-ide\\scratch\\hyperion-web-agent\\fb_en_published_final.png');

  ws.close();
  console.log('\n✅ Facebook Reels (English) PUBLISHED!');
}

main().catch(e => { console.error('❌ FATAL Facebook EN:', e.message); process.exit(1); });
