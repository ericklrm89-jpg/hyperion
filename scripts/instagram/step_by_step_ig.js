/**
 * HYPERION — Step-by-Step Instagram Upload on Open Modal
 */
...
async function main() {
  ...
  console.log('1. Injecting video file into open Instagram modal...');
  ...
  console.log('   Waiting 8s for preview...');
  await wait(8000);

  // Click Next (1)
  console.log('2. Clicking Next (1)...');
  ...
  if (next1.result?.value) {
    ...
    console.log(`   Next 1 at x=${p.x}, y=${p.y}`);
    await mouseClick(ws, p.x, p.y);
  }
  await wait(4000);

  // Click Next (2)
  console.log('3. Clicking Next (2)...');
  ...
  if (next2.result?.value) {
    ...
    console.log(`   Next 2 at x=${p.x}, y=${p.y}`);
    await mouseClick(ws, p.x, p.y);
  }
  await wait(4000);

  // Inject caption
  console.log('4. Writing caption text...');
  ...
  // Click Share
  console.log('5. Clicking Share...');
  const share = await cdpCall(ws, 'Runtime.evaluate', {
    expression: `(() => {
      const b = Array.from(document.querySelectorAll('div[role="button"], button, span')).find(e => {
        const txt = (e.textContent || '').trim().toLowerCase();
  ...
  if (share.result?.value) {
    const p = JSON.parse(share.result.value);
    console.log(`   Share clicked at x=${p.x}, y=${p.y}`);
    await mouseClick(ws, p.x, p.y);
  }
  console.log('   Waiting 12s for upload completion...');
  await wait(12000);

  const ss = await cdpCall(ws, 'Page.captureScreenshot', { format: 'png' });
  fs.writeFileSync('C:\\Users\\erick\\.gemini\\antigravity-ide\\scratch\\hyperion-web-agent\\ig_STEP_BY_STEP_RESULT.png', Buffer.from(ss.data, 'base64'));
  console.log('Saved ig_STEP_BY_STEP_RESULT.png');
  ws.close();
}
