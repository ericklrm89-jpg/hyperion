...
export class LayerDetector {
   ...
   */
  async filterActiveLayer(elements: Array<{ x: number; y: number }>): Promise<Array<{ x: number; y: number }>> {
    const layer = await this.detect()
    if (!layer.dialog) return elements // no dialog, show all

    const d = layer.dialog
    return elements.filter(el =>
      el.x >= d.x && el.x <= d.x + d.w &&
