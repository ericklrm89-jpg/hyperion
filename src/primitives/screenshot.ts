...
export class ScreenshotPrimitive {
  ...
  async capture(options: ScreenshotOptions = {}): Promise<Buffer> {
    ...
    try {
      ...
      return Buffer.from((result as unknown as ScreenshotResult).data, 'base64')
    } catch (err: any) {
      throw new Error(`Screenshot failed: ${err.message}`)
    }
  }

  private async captureFullPage(format: string, quality?: number): Promise<Buffer> {
    const dims = await this.cxn.evaluate(
      '(function(){return JSON.stringify({w:document.documentElement.scrollWidth,h:document.documentElement.scrollHeight})})()'
    )
    if (!dims?.value) throw new Error('Could not get page dimensions')
    const { w, h } = JSON.parse(dims.value)
    if (w <= 0 || h <= 0) throw new Error('Invalid page dimensions')
  ...
  private async captureElement(selector: string, format: string, quality?: number): Promise<Buffer> {
    const escaped = selector.replace(/\\/g, '\\\\').replace(/'/g, "\\'")
    const rectResult = await this.cxn.evaluate(
      '(function(){var el=document.querySelector(\'' + escaped + '\');if(!el)return null;var r=el.getBoundingClientRect();return JSON.stringify({x:r.left+window.scrollX,y:r.top+window.scrollY,w:r.width,h:r.height})})()'
    )
    if (!rectResult?.value) throw new Error('Element not found: ' + selector)
  ...
  async compare(actual: Buffer, expected: Buffer, threshold = 0.1): Promise<{ diff: number; diffPixels: number }> {
