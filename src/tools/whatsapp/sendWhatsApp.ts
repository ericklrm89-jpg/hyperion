...
export async function executeSendWhatsApp(
  ...
  if (input.mediaPath) {
    logger.info({ media: input.mediaPath }, '[WhatsApp] Inyectando adjunto multimedia HD mediante DOM.setFileInputFiles');
    ...
    const evalInput = await cxn.evaluate(`document.querySelector('input[accept*="image"]') || document.querySelector('input[type="file"]')`) as any;
    const objectId = evalInput?.objectId;

    if (objectId) {
      const desc = await cxn.call('DOM.describeNode', { objectId }) as any;
