...
export const postToInstagramSchema = z.object({
  filePath: z.string().min(1, 'File path is required').describe('Absolute path to image or video file'),
  caption: z.string().optional().describe('Post caption and hashtags'),
  isReel: z.boolean().default(false).describe('Whether this is a Reel (video) or standard feed post'),
});
 ...
 */
export const postToInstagramAction: ActionDefinition<typeof postToInstagramSchema> = {
  id: 'instagram-post',
 ...
 */
export async function executePostToInstagram(
  cxn: ConnectionManager,
  rawInput: PostToInstagramRawInput,
  options?: PostToInstagramOptions
): Promise<VerifiedActionResult> {
  const input = postToInstagramSchema.parse(rawInput);
  const sleep = options?.sleepFn || ((ms: number) => new Promise(r => setTimeout(r, ms)));

  logger.info({ filePath: input.filePath, isReel: input.isReel }, '[Instagram] Starting automated post flow');
  ...
  if (!queryRes?.nodeId) {
    logger.warn('[Instagram] input[type="file"] not found via DOM.querySelector, attempting fallback selector');
  } else {
    await cxn.call('DOM.setFileInputFiles', {
      files: [input.filePath],
      nodeId: queryRes.nodeId,
    });
    logger.info('[Instagram] File injected directly into DOM');
  }
  await sleep(2500);
  ...
  logger.info('[Instagram] Post action dispatched. Starting post-action visual verification...');
