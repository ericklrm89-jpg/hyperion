...
export class MasterDashboard {
  // ANSI Color Tokens
  private static C_RESET = '\x1b[0m';
  private static C_BOLD = '\x1b[1m';
  private static C_CYAN = '\x1b[1;36m';
  private static C_MAGENTA = '\x1b[1;35m';
  private static C_GREEN = '\x1b[1;32m';
  private static C_YELLOW = '\x1b[1;33m';
  private static C_RED = '\x1b[1;31m';
  private static C_BLUE = '\x1b[1;94m';
  ...
  static render(state: DashboardViewState): void {
    ...
    if (state.sessions.length === 0) {
      ...
      state.sessions.forEach((s, idx) => {
        ...
        if (s.status === HealthState.OFFLINE) statusStr = '🔴 OFFLINE  ';
        else if (s.status === HealthState.RECONNECTING) statusStr = '🟡 RECONECT ';
    ...
    state.sessions.forEach((s) => {
      if (s.status === HealthState.OFFLINE) {
  ...
  private static getTabIcon(url: string, title: string): string {
    const combined = (url + ' ' + title).toLowerCase();
    if (combined.includes('whatsapp')) return '💬';
    if (combined.includes('mail.google') || combined.includes('gmail')) return '✉️ ';
    if (combined.includes('instagram')) return '📸';
    if (combined.includes('facebook')) return '👥';
    if (combined.includes('tiktok')) return '🎵';
    if (combined.includes('gemini') || combined.includes('claude') || combined.includes('chatgpt')) return '🤖';
    if (combined.includes('crm') || combined.includes('lead')) return '📊';
    if (combined.includes('fairdraw')) return '💎';
    return '🌐';
  }
}
