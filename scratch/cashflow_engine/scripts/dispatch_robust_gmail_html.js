...
async function run() {
  ...
  ws.on('open', async () => {
    ...
    console.log('⏳ Esperando carga completa de la bandeja de entrada...');
    for (let i = 0; i < 20; i++) {
      ...
      if (chk.result && chk.result.value) {
        console.log(`✅ Interfaz Gmail lista en ${i+1}s.`);
        break;
      }
    }
    ...
    // Polling hasta que aparezca el compose box
    ...
    for (let i = 0; i < 15; i++) {
      ...
      if (chk.result && chk.result.value) {
        console.log(`✅ Compose Box listo en ${i+1} intentos.`);
        break;
      }
    }

    console.log('4. Maximizando ventana de redacción para vista completa...');
    await call('Runtime.evaluate', {
      expression: `(() => {
        const maxBtn = document.querySelector('img[alt="Pantalla completa"]') || 
                       document.querySelector('div[aria-label="Maximizar"]') ||
                       document.querySelector('img[aria-label="Pantalla completa"]');
        if (maxBtn) maxBtn.click();
      })()`
    });
    await new Promise(r => setTimeout(r, 1000));

    console.log('5. Inyectando Destinatario y Asunto...');
    await call('Runtime.evaluate', {
      expression: `(() => {
    ...
    console.log('6. Inyectando plantilla HTML visual dentro del editor...');
    await call('Runtime.evaluate', {
      expression: `(() => {
