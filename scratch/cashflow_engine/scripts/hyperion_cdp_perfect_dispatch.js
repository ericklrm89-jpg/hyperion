...
async function run() {
  ...
  ws.on('open', async () => {
    ...
    // Polling hasta que cargue la interfaz principal de Gmail
    console.log('⏳ Esperando carga de Gmail...');
    for (let i = 0; i < 20; i++) {
      await new Promise(r => setTimeout(r, 1000));
      const chk = await call('Runtime.evaluate', {
        expression: `!!(document.querySelector('div[gh="cm"]') || document.querySelector('div.T-I.T-I-KE.L3'))`,
        returnByValue: true
      });
    ...
    console.log('4. Escribiendo Destinatario nativamente con teclado CDP...');
    await call('Runtime.evaluate', {
      expression: `(() => {
    ...
    // Escribir destinatario nativamente
    ...
    console.log('6. Inyectando cuerpo HTML visual enriquecido con formato de tabla...');
    await call('Runtime.evaluate', {
      expression: `(() => {
    ...
    console.log('8. Capturando auditoría visual del correo renderizado antes de enviar...');
    ...
    console.log('9. Enviando correo con Ctrl+Enter...');
    await call('Runtime.evaluate', {
      expression: `(() => {
