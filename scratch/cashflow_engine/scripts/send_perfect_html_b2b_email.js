...
const EMAIL_DATA = {
  ...
  body_html: `<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #0f172a; max-width: 660px; line-height: 1.6; border: 1px solid #cbd5e1; border-radius: 16px; padding: 26px; background: #ffffff; margin: 0 auto; box-shadow: 0 10px 25px -5px rgba(0,0,0,0.08);">
...
async function run() {
  console.log('⚡ [Hyperion 3.5] Iniciando envío de correo HTML enriquecido con Capa Manus...');
  ...
  ws.on('open', async () => {
    ...
    // 3. Escribir Destinatario
    console.log('3. Escribiendo Destinatario nativamente...');
    await call('Runtime.evaluate', {
      expression: `(() => {
    ...
    // 4. Escribir Asunto
    console.log('4. Escribiendo Asunto...');
    await call('Runtime.evaluate', {
      expression: `(() => {
    ...
    // 5. Inyectar HTML real con Selection y execCommand insertHTML
    console.log('5. Inyectando plantilla HTML enriquecida con tabla B2B...');
    await call('Runtime.evaluate', {
      expression: `(() => {
    ...
    // 7. Capturar captura previa del correo con su tabla visible
    console.log('7. Capturando vista previa HD del correo con tabla...');
    ...
    // 10. Abrir el correo enviado arriba
    console.log('10. Abriendo el correo recién enviado para auditoría visual...');
    await call('Runtime.evaluate', {
      expression: `(() => {
    ...
    // 11. Captura final del correo abierto
    const postSnap = await call('Page.captureScreenshot', { format: 'jpeg', quality: 95 });
    if (postSnap && postSnap.data) {
      const outPost = path.join(ASSETS_DIR, 'live_gm_opened_proposal_verified.jpg');
