...
async def send_proposal():
    ...
    print(f"🎯 Conectando a Gmail Tab: {gm_tab.get('title')}")

    async with websockets.connect(gm_tab['webSocketDebuggerUrl'], max_size=50*1024*1024) as ws:
        # 1. Cerrar popup de alerta si existe
        js_dismiss = """(() => {
            const btn = Array.from(document.querySelectorAll('button')).find(b => b.innerText && b.innerText.includes('Aceptar'));
            if (btn) { btn.click(); return 'ALERT_DISMISSED'; }
            return 'NO_ALERT';
        })()"""
        await ws.send(json.dumps({'id': 1, 'method': 'Runtime.evaluate', 'params': {'expression': js_dismiss, 'returnByValue': True}}))
        r = json.loads(await ws.recv())
        print("Alerta previa:", r.get('result', {}).get('value'))
        await asyncio.sleep(1)

        # 2. Click Redactar si no hay compose abierto
        js_open_compose = """(() => {
            ...
            const redactar = Array.from(document.querySelectorAll('div[role="button"]')).find(b => b.innerText && (b.innerText.includes('Redactar') || b.innerText.includes('Compose')));
            if (redactar) { redactar.click(); return 'REDACTAR_CLICKED'; }
            return 'NOT_FOUND';
        })()"""
        await ws.send(json.dumps({'id': 2, 'method': 'Runtime.evaluate', 'params': {'expression': js_open_compose, 'returnByValue': True}}))
        r = json.loads(await ws.recv())
        print("Estado Compose:", r.get('result', {}).get('value'))
        await asyncio.sleep(2)

        # 3. Llenar campos correctamente
        js_fill_fields = f"""(() => {{
            // Para
            const toInput = document.querySelector('input[aria-label="Para"]') || document.querySelector('input.agP.aFw') || document.querySelector('input[peoplekit-id]');
            if (toInput) {{
                toInput.focus();
            ...
            return 'FIELDS_INJECTED';
        }})()"""
        await ws.send(json.dumps({'id': 3, 'method': 'Runtime.evaluate', 'params': {'expression': js_fill_fields, 'returnByValue': True}}))
        r = json.loads(await ws.recv())
        print("Inyección campos:", r.get('result', {}).get('value'))
        await asyncio.sleep(2)

        # 4. Adjuntar flyer HD
        if os.path.exists(FLYER):
            print(f"📎 Inyectando flyer adjunto HD: {FLYER}...")
            ...
            if node_id:
                ...
                if b_id:
                    ...
                    await ws.recv()
                    print("Adjunto inyectado.")
                    await asyncio.sleep(4)

        # 5. Captura previa al envío (muestra el correo renderizado con su tabla y diseño)
        ...
        js_send = """(() => {
            const sendBtn = Array.from(document.querySelectorAll('div[role="button"]')).find(b => b.innerText && (b.innerText.trim() === 'Enviar' || b.innerText.trim() === 'Send' || b.innerText.includes('Enviar')));
        ...
        if data:
            ...
            with open(final_out, 'wb') as f:
                f.write(base64.b64decode(data))
            print(f"✅ CAPTURA CONFIRMACIÓN ENVIADOS GUARDADA: {final_out}")

if __name__ == '__main__':
    asyncio.run(send_proposal())
