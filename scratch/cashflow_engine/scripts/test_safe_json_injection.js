...
async function run() {
  ...
  ws.on('open', async () => {
    ...
    console.log('Inyectando HTML de forma segura con JSON.stringify...');
    const script = `(() => {
      const editor = document.querySelector('div.editable[aria-label="Cuerpo del mensaje"]');
      if (editor) {
        editor.focus();
        editor.innerHTML = ${JSON.stringify(HTML_OFFER)};
        editor.dispatchEvent(new Event('input', { bubbles: true }));
        editor.dispatchEvent(new Event('change', { bubbles: true }));
        return { success: true, length: editor.innerHTML.length, innerText: editor.innerText };
      }
      return { success: false, reason: 'NO_EDITOR' };
    })()`;

    const res = await call('Runtime.evaluate', { expression: script, returnByValue: true });
    console.log('RESULTADO INYECCIÓN:', JSON.stringify(res.result?.value, null, 2));

    await new Promise(r => setTimeout(r, 1200));

    // Capturar pantalla limpia
    ...
    if (snap && snap.data) {
      ...
      console.log('✅ CAPTURA CON HTML RENDERIZADO GUARDADA:', out);
    }
