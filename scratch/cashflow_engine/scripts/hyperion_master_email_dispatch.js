...
async function run() {
  console.log('⚡ [Hyperion 3.5] Iniciando protocolo maestro de envío de propuesta HTML...');
  const tabs = await getTabs();
  const gmTab = tabs.find(t => t.type === 'page' && t.url.includes('mail.google.com/mail/u/'));

  if (!gmTab) {
    console.error('❌ No se encontró la pestaña de Gmail activa.');
    return;
  }

  console.log(`🎯 Pestaña conectada: [${gmTab.id}] ${gmTab.title}`);
  ...
  ws.on('open', async () => {
    try {
      ...
      // 1. Inyectar Capa Manus
      console.log('🎨 Inyectando Capa Manus Dinámica...');
      await cdpCall(ws, 'Runtime.evaluate', { expression: CAPA_MANUS_SCRIPT, returnByValue: true });
      await new Promise(r => setTimeout(r, 1000));

      // 2. Cerrar diálogos modales previos de error si existen
      await cdpCall(ws, 'Runtime.evaluate', {
        expression: `(() => {
          const btn = Array.from(document.querySelectorAll('button')).find(b => b.innerText && (b.innerText.includes('Aceptar') || b.innerText.includes('OK')));
          if (btn) btn.click();
        })()`,
        returnByValue: true
      });
      ...
      // 3. Abrir ventana de Redactar
      console.log('📝 Abriendo ventana de Redactar...');
      const redactarRes = await cdpCall(ws, 'Runtime.evaluate', {
        expression: `(() => {
          const composeBox = document.querySelector('div[aria-label="Cuerpo del mensaje"]') || document.querySelector('div[role="textbox"]');
          if (composeBox) return 'ALREADY_OPEN';
          const redactar = document.querySelector('div[gh="cm"]') ||
          ...
          if (redactar) { redactar.click(); return 'CLICKED_REDACTAR'; }
          return 'NOT_FOUND';
        })()`,
        returnByValue: true
      });
      console.log('Estado Redactar:', redactarRes.result?.value);
      await new Promise(r => setTimeout(r, 2500));

      // 4. Inyectar Destinatario, Asunto y Cuerpo HTML con formato enriquecido
      console.log('✍️ Inyectando Destinatario, Asunto y Plantilla HTML B2B...');
      const injectRes = await cdpCall(ws, 'Runtime.evaluate', {
        expression: `(() => {
          // Destinatario
      ...
      // 5. Inyectar Flyer HD Adjunto
      if (fs.existsSync(FLYER)) {
        console.log(`📎 Inyectando archivo adjunto HD: ${FLYER}...`);
        const doc = await cdpCall(ws, 'DOM.getDocument', { depth: -1, pierce: true });
        const fileInput = await cdpCall(ws, 'DOM.querySelector', { nodeId: doc.root.nodeId, selector: 'input[type="file"]' });
        if (fileInput && fileInput.nodeId) {
          const desc = await cdpCall(ws, 'DOM.describeNode', { nodeId: fileInput.nodeId });
          if (desc.node && desc.node.backendNodeId) {
            await cdpCall(ws, 'DOM.setFileInputFiles', { backendNodeId: desc.node.backendNodeId, files: [FLYER] });
            console.log('✅ Archivo adjunto procesado exitosamente.');
            await new Promise(r => setTimeout(r, 4000));
          }
        }
      }

      // 6. Captura de Auditoría Previa al Envío (Muestra el correo enriquecido con Capa Manus)
      console.log('📸 Capturando auditoría visual previa al envío...');
      const preSnap = await cdpCall(ws, 'Page.captureScreenshot', { format: 'jpeg', quality: 95 });
      if (preSnap && preSnap.data) {
        ...
        console.log(`🖼️ Captura previa guardada: ${outPre}`);
      }

      // 7. Enviar correo (mediante click o Ctrl+Enter)
      console.log('🚀 Enviando correo formal...');
      const sendRes = await cdpCall(ws, 'Runtime.evaluate', {
        expression: `(() => {
      ...
      console.log('Resultado Enviar:', sendRes.result?.value);
      await new Promise(r => setTimeout(r, 6000));

      // 8. Navegar a Enviados (#sent) para verificación
      console.log('📬 Navegando a Enviados (#sent) para auditoría de entrega...');
      await cdpCall(ws, 'Page.navigate', { url: 'https://mail.google.com/mail/u/0/#sent' });
      await new Promise(r => setTimeout(r, 4000));

      // Inyectar Capa Manus en #sent
      await cdpCall(ws, 'Runtime.evaluate', { expression: CAPA_MANUS_SCRIPT, returnByValue: true });
      await new Promise(r => setTimeout(r, 1500));

      // 9. Captura final de auditoría
      const postSnap = await cdpCall(ws, 'Page.captureScreenshot', { format: 'jpeg', quality: 95 });
      if (postSnap && postSnap.data) {
        ...
        console.log(`✅ CAPTURA AUDITADA FINAL GUARDADA: ${outPost}`);
      }

      console.log('🎉 [Hyperion] Protocolo de propuesta B2B completado con éxito.');
      ws.close();
      process.exit(0);

    } catch (err) {
      console.error('❌ Error en ejecución Hyperion:', err);
      ws.close();
      process.exit(1);
    }
  });
