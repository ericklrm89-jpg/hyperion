...
async function run() {
  ...
  ws.on('open', async () => {
    ...
    console.log('1. Inyectando Capa Manus v3.1...');
    const evalRes = await call('Runtime.evaluate', { expression: ROBUST_MANUS_ENGINE, returnByValue: true });
    console.log('Eval Result:', JSON.stringify(evalRes));

    await new Promise(r => setTimeout(r, 1200));

    console.log('2. Tomando captura...');
