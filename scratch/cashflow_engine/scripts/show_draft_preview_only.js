...
<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #0f172a; max-width: 660px; line-height: 1.6; border: 1px solid #cbd5e1; border-radius: 16px; padding: 26px; background: #ffffff; margin: 0 auto; box-shadow: 0 10px 25px -5px rgba(0,0,0,0.08);">
  ...
  <!-- Header Branding -->
  <table style="width: 100%; border-bottom: 2px solid #0f172a; padding-bottom: 14px; margin-bottom: 20px;">
    <tr>
      <td>
...
async function run() {
  console.log('⚡ Iniciando renderizado de propuesta en tiempo real (SIN enviar)...');
  ...
  ws.on('open', async () => {
    ...
    console.log('1. Inyectando Capa Manus...');
    await call('Runtime.evaluate', { expression: CAPA_MANUS_SCRIPT });
    await new Promise(r => setTimeout(r, 600));

    // 2. Hacer clic en [1] Aceptar para cerrar el popup de error
    console.log('2. Haciendo clic en el botón Aceptar del popup de error...');
    const dismissRes = await call('Runtime.evaluate', {
      expression: `(() => {
        const btn = document.querySelector('button[name="ok"]') ||
                    Array.from(document.querySelectorAll('button')).find(b => b.innerText && (b.innerText.includes('Aceptar') || b.innerText.includes('OK')));
        if (btn) {
          btn.click();
          return 'ACEPTAR_CLICKED';
        }
        return 'NO_POPUP';
      })()`,
      returnByValue: true
    });
    console.log('Estado Aceptar:', dismissRes.result?.value);
    await new Promise(r => setTimeout(r, 1000));

    // 3. Cerrar borradores residuales viejos
    console.log('3. Limpiando borradores viejos...');
    await call('Runtime.evaluate', {
      expression: `(() => {
        document.querySelectorAll('img[alt="Guardar y cerrar"], div[aria-label="Guardar y cerrar"], div.Ha').forEach(b => b.click());
      })()`
    });
    await new Promise(r => setTimeout(r, 1200));

    // 4. Hacer clic en Redactar
    console.log('4. Haciendo clic en Redactar...');
    await call('Runtime.evaluate', {
      expression: `(() => {
        const btn = document.querySelector('div[gh="cm"]') || document.querySelector('div.T-I.T-I-KE.L3');
        if (btn) btn.click();
      })()`
    });
    ...
    for (let i = 0; i < 15; i++) {
      ...
      const chk = await call('Runtime.evaluate', {
        expression: `!!(document.querySelector('div[role="textbox"]'))`,
        returnByValue: true
      });
      if (chk.result && chk.result.value) break;
    }

    // 5. Maximizar ventana de redacción para vista amplia
    console.log('5. Maximizando ventana de redacción...');
    await call('Runtime.evaluate', {
      expression: `(() => {
        const maxBtn = document.querySelector('img[alt="Pantalla completa"]') || 
                       document.querySelector('div[aria-label="Maximizar"]') ||
                       document.querySelector('div.aI8');
        if (maxBtn) maxBtn.click();
      })()`
    });
    await new Promise(r => setTimeout(r, 800));

    // 6. Escribir destinatario nativamente y generar chip
    console.log('6. Escribiendo Destinatario (erickl.rm@gmail.com)...');
    await call('Runtime.evaluate', {
      expression: `(() => {
    ...
    await call('Input.insertText', { text: EMAIL_DATA.to });
    ...
    // 7. Escribir Asunto
    console.log('7. Escribiendo Asunto...');
    await call('Runtime.evaluate', {
      expression: `(() => {
    ...
    // 8. Inyectar HTML real directamente en el editor
    console.log('8. Inyectando estructura HTML con tabla de nómina y formato visual...');
    await call('Runtime.evaluate', {
      expression: `(() => {
    ...
    if (fs.existsSync(FLYER)) {
      console.log(`9. Inyectando archivo adjunto HD: ${FLYER}...`);
    ...
    // 10. Inyectar Capa Manus y tomar captura en tiempo real (SIN ENVIAR)
    console.log('10. Inyectando Capa Manus para captura visual...');
    await call('Runtime.evaluate', { expression: CAPA_MANUS_SCRIPT });
    await new Promise(r => setTimeout(r, 1000));
    ...
    if (snap && snap.data) {
      ...
      console.log('✅ CAPTURA EN TIEMPO REAL GUARDADA:', out);
    }
